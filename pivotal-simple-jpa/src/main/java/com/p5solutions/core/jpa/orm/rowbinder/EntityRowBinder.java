...
public class EntityRowBinder<T> implements RowMapper<T> {
	 ...
	 *          the entity parser
	 */
	public EntityRowBinder(Class<T> entityClass, EntityUtility entityUtility, MapUtility mapUtility,
			String currentQueryIdentifier, DependencyJoinFilter joinFilter, EntityParser entityParser) {
		...
		if (this.entityDetail == null) {
			String error = EntityUtility.class + " returned a null " + EntityDetail.class
					+ ", please make sure the entity is valid, or check the " + EntityUtility.class + " cache.";
			logger.error(error);
	...
	public T mapRow(ResultSet rs, int rowNum) throws SQLException {
		...
		for (ParameterBinder pb : entityDetail.getParameterBinders()) {
			...
			if (columnIndex == -1) {
				NoColumnFoundInResultSetException e = new NoColumnFoundInResultSetException(entityDetail.getEntityClass(),
						columnName);
			...
			if (doColumn(pb, entity, resultValue)) {
				...
				if (isDebug) {
					logger.debug(" -> Column [" + metaData.getColumnName(columnIndex) + "] with value ["
