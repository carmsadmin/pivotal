...
/**
 * The Class MapUtility. This class will either map a value to a target object defined by a specific path, or it will
 * get a specific object instance from a given target object by the defined path.
 * 
 * @author Kasra Rasaee
 * @since 2010-11-10
 * 
...
public class MapUtilityImpl implements MapUtility {
	...
	/**
	 * @see com.p5solutions.core.jpa.orm.MapUtility#map(com.p5solutions.core.jpa.orm.ParameterBinder, java.lang.Object, java.lang.Object,
	 *      java.lang.String)
	 */
	@Override
	public Object map(ParameterBinder pb, Object target, Object value, String bindingPath) {
		...
		if (methods != null) {
			...
			if (p.length == 1) {
				...
				try {
					...
					throw new RuntimeException("Unable map data due to conversion problem, please check target class "
							+ targetClazz + " for the problem when binding path " + bindingPath, e);
				}
		 ...
		 * 
		 * // get the field name from index 0, since we only splice by a // maximum of 2 String fieldName = p[0];
		 * 
		 * // find the getter method, make sure it exists Method getterMethod = ReflectionUtility.findGetterMethod(
		 * targetClazz, fieldName); if (getterMethod == null) { throw new RuntimeException(new NoSuchMethodException(
		 * "No getter method found when using field name [" + fieldName + "] as search pattern!")); }
		 * 
		 * // find the setter method, make sure it exists Method setterMethod = ReflectionUtility.findSetterMethod(
		 * targetClazz, getterMethod); if (setterMethod == null) { throw new RuntimeException(new NoSuchMethodException(
		 * "No setter method found when using field name [" + fieldName + "] as search pattern!")); }
		 * 
		 * // whats the return type of the getter method? Class<?> targetType = getterMethod.getReturnType();
		 * 
		 * // if the length of p is equal == to 1, then we've hit the end // of the path if (p.length == 1) { // convert the
		 * parameter type value = convertValue(value, targetType);
		 * 
		 * // finally set the value at the last path ReflectionUtility.setValue(setterMethod, target, value); } else if
		 * (p.length == 2) {
		 * 
		 * // if p length == to 2 the, continue to recursively walk the // graph. Class<?> nextClass =
		 * getterMethod.getReturnType(); Object nextTarget = ReflectionUtility.getValue(getterMethod, target); if
		 * (nextTarget == null) { nextTarget = ReflectionUtility.newInstance(nextClass); ReflectionUtility
		 * .setValue(setterMethod, target, nextTarget); }
		 * 
		 * // try to map the value to the next available path return map(nextTarget, value, p[1]); } } return target;
		 */
	}
	...
	protected Method[] doo(Class<?> targetClazz, String[] p) {
		...
		if (p.length > 0) {
			...
			if (getterMethod == null) {
				throw new RuntimeException(new NoSuchMethodException("No getter method found when using field name ["
			...
			if (setterMethod == null) {
				throw new RuntimeException(new NoSuchMethodException("No setter method found when using field name ["
						+ fieldName + "] as search pattern!"));
			}
	...
	/**
	 * @see com.p5solutions.core.jpa.orm.MapUtility#get(com.p5solutions.core.jpa.orm.ParameterBinder, java.lang.Object, java.lang.String)
	 */
	@Override
	 ...
	 * @return the object
	 */
	protected Object convertValue(ParameterBinder pb, Object value, String bindingPath, Class<?> targetType)
