...
public class EntityDetail<T> {
	...
	public MappedSuperclass getMappedSuperclassAnnotation() {
		return ReflectionUtility.findAnnotation(getEntityClass(),
...
//		entityKey.setBindingValues(bindingValues);
	 ...
	 * 
	 * Note, if premitives are used instead, such as int, instead of Integer, 
	 * then this could give negative results. Highly recommend using Integer 
	 * instead of int, or Boolean instead of boolean; despite it being boxed.
	 * 
	...
	/**
	 * Gets the parameter binder by the type (Join(J) or Column(C)) filtering out by columnName
	 *
	 * @param type the type
	...
	protected ParameterBinder getParameterBinder(String type, String name, boolean ignoreCase) {
		if (Comparison.isEmpty(name)) {
			throw new NullPointerException(
					"Cannot find null or empty column name in Parameter Binder Cache!");
		}
