...
public class EntityPersistUtility {
  ...
  protected <T> void throwTableEntityNullException(Class<T> clazz, Table table, Entity entity) {
    ...
    if (entity == null || table == null) {
      throw new NullPointerException("Class type " + clazz + " must be annotated with @" + Entity.class + " and @" + Table.class
          + " when building basic SELECT / DML operations.");
    }
  }
  ...
  protected StringBuilder appendFieldValueUpdate(ParameterBinder pb, StringBuilder fieldsValues, String tableAlias) {
    ...
    if (columnName != null) {
      ...
      // append a table alias to the column if one exists, perhaps in a MERGE
      // statement, for example UPDATE TABLE A SET A.COL = :BB
      if (Comparison.isNotEmpty(tableAlias)) {
  ...
  protected <T> void buildMergeDML(Class<T> entityClass) throws Exception {
    ...
    mergeDML += "UPDATE SET " + buildUpdateSET(entityDetail, "b"); // b = table
                                                                   // alias of
                                                                   // merge
                                                                   // statement
    ...
    mergeDML += buildInsertDML(entityClass, null, true, "b"); // b = table alias
                                                              // of merge
                                                              // statement
