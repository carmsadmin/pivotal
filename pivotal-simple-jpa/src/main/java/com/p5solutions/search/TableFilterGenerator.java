...
public class TableFilterGenerator implements FilterGenerator<TableFilterGeneratorResult> {
   ...
   */
  protected ProcessorResult tableDefintions(FilterChain chain, Map<String, FilterJunction> duplicateFilter, StringBuilder tableSQL, FilterJunction junction,
      FilterJunction previous, ProcessorResult processorResult, FilterGeneratorResult result) {
  ...
  /**
   * Appends to the WHERE definition a condition and value, e.g. column_nm =
   * :value
   * 
   * @param result
   *          the result
   ...
   *          the value
   */
  protected void whereDefinition(TableFilterGeneratorResult result, String condition, StringBuilder whereSQL, String bindingPath, Object value) {
   ...
   *          the values
   */
  protected void whereDefinitionNotIN(TableFilterGeneratorResult result, StringBuilder whereSQL, String bindingPath, List<Object> values) {

    // NOT IN (...)
    whereDefinitionINorNOT(true, result, whereSQL, bindingPath, values);
  }
   ...
   *          the values
   */
  protected void whereDefinitionIN(TableFilterGeneratorResult result, StringBuilder whereSQL, String bindingPath, List<Object> values) {

    // IN (...)
    whereDefinitionINorNOT(false, result, whereSQL, bindingPath, values);
  }
   ...
   *          the values
   */
  protected void whereDefinitionINorNOT(boolean isNot, TableFilterGeneratorResult result, StringBuilder whereSQL, String bindingPath, List<Object> values) {
   ...
   *          the value2
   */
  protected void whereDefinitionBetween(TableFilterGeneratorResult result, StringBuilder whereSQL, String bindingPath, Object value1, Object value2) {
   ...
   *          the result
   */
  protected void whereClause(FilterChain chain, StringBuilder finalSQL, StringBuilder tableSQL, StringBuilder whereSQL, FilterJunction junction,
      FilterJunction previous, ProcessorResult processorResult, TableFilterGeneratorResult result) {
    ...
    if (Comparison.isNotNull(basicCondition)) {
      ...
      whereDefinitionNotIN(result, whereSQL, path, criteriaValue.getValues());
    } else if (condition.isBetween()) {
      whereDefinitionBetween(result, whereSQL, path, criteriaValue.getValue(0), criteriaValue.getValue(1));
    }
   ...
   * @return the processor result
   */
  protected ProcessorResult iterateJunctions(FilterChain chain, Map<String, FilterJunction> duplicateFilter, StringBuilder finalSQL, StringBuilder tableSQL,
      StringBuilder whereSQL, List<FilterJunction> junctions, FilterJunctionComposite previousComposite, ProcessorResult processorResult,
      TableFilterGeneratorResult result) {

    // if there are no junctions, then its an empty composite.
    if (junctions == null) {
      return processorResult;
    }
    
    // TODO determine if this is the best method to figure out the Operator to
    // use
    // in the next set of junctions,.. e.g. if there was in-fact a previous
    // iteration
    // of junctions, then the first junction->operator within this set of
    // Junctions
    // should be used to determine the Operator, e.g. (previous->junctions) AND?
    // (next->junctions)
   ...
   * @return the processor result
   */
  protected ProcessorResult iterateComposites(FilterChain chain, StringBuilder finalSQL, //
      StringBuilder tableSQL, //
      StringBuilder whereSQL, List<FilterJunctionComposite> composites, //
      ProcessorResult processorResult, TableFilterGeneratorResult result) {
    ...
    for (FilterJunctionComposite composite : composites) {
      ...
      processorResult = iterateJunctions(chain, filtering, finalSQL, tableSQL, whereSQL, junctions, previousComposite, processorResult, result);
  ...
  public <T extends AbstractEntity> List<T> processResult(Class<T> returnEntityClass, String[] filterColumns, FilterChain chain,
      TableFilterGeneratorResult result) {
    ...
    if (table == null) {
      throw new NullPointerException("Class of type " + returnEntityClass + " is not annotated with type " + Table.class
          + ", thus no table name can be retrieved to join against the filter chains resultset.");
    }
