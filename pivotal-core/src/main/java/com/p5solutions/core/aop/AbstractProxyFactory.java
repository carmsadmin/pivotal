...
/**
 * AbstractProxyFactory: An abstract factory for creating proxy objects. Use
 * this as a base for all proxy factories.
 * 
 * @author Kasra Rasaee
 * @since 2009-02-06
...
public abstract class AbstractProxyFactory implements com.p5solutions.core.aop.ProxyFactory {
  ...
  private void throwIsNotInterface(Class<?> proxyInterfaceClazz) {
    if (!proxyInterfaceClazz.isInterface()) {
      throw new RuntimeException("Argument proxyInterfaceClazz must be of " + "type interface not concrete or abstract class!");
    }
  }
   ...
   * @param paramClazzes
   */
  private void throwSameMethod(Class<?> proxyInterfaceClazz, Class<?> pojoClazz, String methodName, Class<?>[] paramClazzes) {
  ...
  /**
   * Creates a new proxy object by the class type, and register the method
   * handler of choice.
   * 
   * @param clazz
   *          the clazz
   ...
   *          the handler
   * 
   * @return the T representing the proxy instance subclassed from the clazz
   *         passed in
   */
  @SuppressWarnings("unchecked")
  protected <T> T createProxy(Class<T> clazz, MethodHandler handler) throws IllegalAccessException, InstantiationException {
    ...
    // set the method handler/invoker (equivalent to cglib's
  ...
  protected <T> Class<T> createProxyClass(Class<T> clazz, MethodInterceptor callback) {
    ...
    try {
      // TODO: what is this in javassist? Enhancer.registerCallbacks(newClazz,
